...
cursorObj.execute('''CREATE TABLE ActualizacionUno(jugador TEXT, edad INT, pais TEXT, equipos TEXT, dorsal INT, posicion TEXT, precio FLOAT, actualizacion TEXT)''')
con.commit()
...
def pasarAdb(players,price,dorsales,paises,edades,posiciones,equipo,actualizacion):
    ...
    i=0
    while i < len(players):
        con.execute('''INSERT INTO ActualizacionUno(jugador,edad,pais,equipos,dorsal,posicion,precio,actualizacion) VALUES(?, ?, ?, ?, ?, ?, ?, ?)''',

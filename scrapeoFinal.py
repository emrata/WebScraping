...
cursorObj.execute('''CREATE TABLE JUGADORES(jugador TEXT, edad INT, pais TEXT, equipos TEXT, dorsal INT, posicion TEXT, precio FLOAT, actualizacion TEXT)''')
con.commit()


def pasarAdb(players,price,dorsales,paises,edades,posiciones,equipo,actualizacion):
    ...
    i=0
    while i < len(players):
        con.execute('''INSERT INTO JUGADORES(jugador,edad,pais,equipos,dorsal,posicion,precio,actualizacion) VALUES(?, ?, ?, ?, ?, ?, ?, ?)''',
                  (players[i], edades[i], paises[i], equipo[i], dorsales[i], posiciones[i], price[i],actualizacion))
        con.commit()
        i=i+1;
...
def scrapeo(url, players, prices, dorsales, paises, edades, posiciones, equipos, playersExcel, valuesExcel, dorsalExcel,
    ...
    for i in pageSoup.find_all("div", {"class": "rn_nummer"}):
        ...
        if dorsal=="-":
            ...
            dorsalExcel.append(dorsal2)
        else:
            dorsales.append(int(dorsal))
            dorsalExcel.append(int(dorsal))
    ...
    {"Players": playersExcel, "Dorsal": dorsalExcel, "Valor": valuesExcel, "Edad": edadExcel, "Pais": paisExcel,
     "Posicion": posicionExcel, "Equipo": equipoExcel, "Actualizacion":actualizacion})
    ...
    pasarAdb(players,prices,dorsales,paises,edades,posiciones,equipos,actualizacion)
